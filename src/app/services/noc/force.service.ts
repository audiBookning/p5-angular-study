...
export class ForceService {
  ...
  bounceMultiple = (element: ElementRef, width: number, height: number): p5 => {
    ...
    return new p5((s: p5) => {
      const generateMover = (i: number) => new Mover(30, 30, i);

      const initMovers = () => {
        movers = new Array<Mover>();

        for (let i = 0; i < 6; i++) movers.push(generateMover((i + 2) * 2));
      };
  ...
  massGravity = (element: ElementRef, width: number, height: number): p5 => {
    ...
    return new p5((s: p5) => {
      ...
      const adjustGravity = (mass: number) => (gravity.y = 0.3 * mass);
  ...
  fluidResistance = (
    ...
    class Liquid {
      ...
      drag = (mover: Mover): p5.Vector => {
        ...
        const dragForce = new p5.Vector().set(
          mover.velocity.x,
          mover.velocity.y
        );
    ...
    return new p5((s: p5) => {
      ...
      const generateMover = (i: number) => {
        ...
        return new Mover(rnd + 20 + (i * s.width) / 9, 0, rnd);
      };

      const initMovers = () => {
        movers = new Array<Mover>();

        for (let i = 0; i < 9; i++) movers.push(generateMover(i));
      };

      const adjustGravity = (mass: number) => (gravity.y = 0.1 * mass);
  ...
  gravity = (element: ElementRef, width: number, height: number): p5 => {
    class Mover {
      ...
      display = (s: p5) => {
        ...
        s.ellipse(this.position.x, this.position.y, 16, 16);

        s.fill(240, 240, 240, 120);

        s.ellipse(this.position.x, this.position.y, 22, 22);
      };
    }
  ...
  movingAttractors = (
    ...
    class Body {
      ...
      display = (s: p5) => {
        ...
        const shade = this.hue + 60 > 360 ? this.hue + 60 - 360 : this.hue + 60;
    ...
    return new p5((s: p5) => {
      const generateBody = (s: p5, id: number) =>
        new Body(
          id,
      ...
      const initBodies = () => {
        ...
        for (let i = 0; i < 3; i++) bodies.push(generateBody(s, i + 1));
      };
  ...
  repellent = (element: ElementRef, width: number, height: number): p5 => {
    class Body extends p5.Vector {
      ...
      display = (s: p5) => {
        ...
        s.ellipse(this.x, this.y, this.size - 8, this.size - 8);
    ...
    return new p5((s: p5) => {
      ...
      const initBodies = () => {
        ...
        for (let i = 0; i < 5; i++) bodies.push(generateBody(s, i + 1));
      };
